...
static bool init_streams(struct ffmpeg_mux *ffm, enum use_video use_video,
		enum track_type track_type)
{
	if (ffm->params.has_video && use_video == USE_VIDEO) {
		create_video_stream(ffm);
	}

	if (ffm->params.tracks && track_type != NO_AUDIO) {
		int tracks = track_type == MULTI_TRACK ? ffm->params.tracks : 1;
...
static inline bool open_output_file(struct ffmpeg_mux *ffm)
{
...
static bool ffmpeg_mux_init_context(struct ffmpeg_mux *ffm,
		enum use_video use_video, enum track_type track_type)
{
	...
	int ret = avformat_alloc_output_context2(&ffm->output, output_format,
			NULL, NULL);
	if (ret < 0) {
	...
	if (!init_streams(ffm, use_video, track_type) ||
	    !open_output_file(ffm)) {
		free_avformat(ffm);
		return false;
	}
		...
		char *argv[])
{
	...
	/* ffmpeg does not have a way of telling how many tracks are supported
	 * for a given output format, so we try each possibility */
	return ffmpeg_mux_init_context(ffm, USE_VIDEO, MULTI_TRACK)  ||
	       ffmpeg_mux_init_context(ffm, USE_VIDEO, SINGLE_TRACK) ||
	       ffmpeg_mux_init_context(ffm, USE_VIDEO, NO_AUDIO)     ||
	       ffmpeg_mux_init_context(ffm, NO_VIDEO,  MULTI_TRACK)  ||
	       ffmpeg_mux_init_context(ffm, NO_VIDEO,  SINGLE_TRACK);
}

static bool ffmpeg_mux_init(struct ffmpeg_mux *ffm, int argc, char *argv[])
{
	ffm->initialized = ffmpeg_mux_init_internal(ffm, argc, argv);
	if (!ffm->initialized) {
		ffmpeg_mux_free(ffm);
		return false;
	}
...
int main(int argc, char *argv[])
{
	...
	if (!ffmpeg_mux_init(&ffm, argc, argv)) {
